...
again:
			...
			case 0:
				...
				rc = stress_run_child(checksum,
						...
						j, started_instances,
						page_size);
				pr_dbg("child process finished and going to exit\n");
				_exit(rc);
			default:
				if (pid > -1) {
					stats->pid = pid;
...
int main(int argc, char **argv, char **envp)
{
	...
	const uint32_t cpus_online = 1;
	const uint32_t cpus_configured = 1;

	int ret;
	bool unsupported = false;		/* true if stressomain
		goto exit_temp_path_free;
	}

	yaml = NULL;

	printf("--exec stressor\n");
	...
	printf("Sanity check minimize/maximize options\n");
	...
	printf("Sanity check seq/all settings\n");
	...
	printf("Sanity check mutually exclusive random seed flags\n");
...
#define read_csr(reg)                 \
    ...
    unsigned long start = read_csr(time);
    unsigned long end;
	if (g_opt_flags & OPT_FLAGS_SEQUENTIAL) {
