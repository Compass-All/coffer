...
void start_single_op()
{
  // printf("start ---------------\n");
  // fflush(stdout);
  last_op_finish_ = now_micros() * 1e-6;
}
...
static void print_vdbe_profile()
{
    FILE *fptr;
  
    char c;
    char *filename = "/vdbe_profile.out";
...
void benchmark_write(bool write_sync, int order, int state,
  ...
  for (int i = 0; i < num_entries; i += entries_per_batch) {
    ...
    for (int j = 0; j < entries_per_batch; j++) {
      ...
      status = sqlite3_step(replace_stmt);  // profiling: check this!
      step_error_check(status);
