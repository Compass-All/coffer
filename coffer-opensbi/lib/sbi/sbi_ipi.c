    ...
    u32 event, void* data)
{
    ...
    if ((SBI_IPI_EVENT_MAX <= event) || !ipi_ops_array[event])
        return SBI_EINVAL;
...
static void sbi_ipi_process_smode(struct sbi_scratch* scratch, struct sbi_trap_regs* regs)
{
...
static void sbi_ipi_process_halt(struct sbi_scratch* scratch, struct sbi_trap_regs* regs)
{
...
void sbi_ipi_process(struct sbi_trap_regs* regs)
{
    ...
    struct sbi_scratch* scratch = sbi_scratch_thishart_ptr();
    const struct sbi_platform* plat = sbi_platform_ptr(scratch);
    struct sbi_ipi_data* ipi_data = sbi_scratch_offset_ptr(scratch, ipi_data_off);
...
int sbi_ipi_init(struct sbi_scratch* scratch, bool cold_boot)
{
    ...
    if (cold_boot) {
        ...
        if (!ipi_data_off)
            return SBI_ENOMEM;
        if (SBI_IPI_EVENT_MAX <= ipi_smode_event || SBI_IPI_EVENT_MAX <= ipi_halt_event)
            return SBI_ENOSPC;
    }
