...
static void sbi_tlb_process_count(struct sbi_scratch* scratch, int count)
{
    ...
    u32 deq_count = 0;
    struct sbi_fifo* tlb_fifo = sbi_scratch_offset_ptr(scratch, tlb_fifo_off);
...
static void sbi_tlb_process(struct sbi_scratch* scratch, struct sbi_trap_regs* regs)
{
    struct sbi_tlb_info tinfo;
    struct sbi_fifo* tlb_fifo = sbi_scratch_offset_ptr(scratch, tlb_fifo_off);
...
{
    unsigned long* tlb_sync = sbi_scratch_offset_ptr(scratch, tlb_sync_off);
...
static int sbi_tlb_update_cb(void* in, void* data)
{
    ...
    if (next->local_fn == sbi_tlb_local_sfence_vma_asid && curr->local_fn == sbi_tlb_local_sfence_vma_asid) {
        if (next->asid == curr->asid)
            ret = __sbi_tlb_range_check(curr, next);
    } else if (next->local_fn == sbi_tlb_local_sfence_vma && curr->local_fn == sbi_tlb_local_sfence_vma) {
        ret = __sbi_tlb_range_check(curr, next);
    }
...
int sbi_tlb_init(struct sbi_scratch* scratch, bool cold_boot)
{
    ...
    if (cold_boot) {
        ...
        if (!tlb_sync_off || !tlb_fifo_off || !tlb_fifo_mem_off)
            return SBI_ENOMEM;
