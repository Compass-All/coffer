...
#include <util/cpu.h>
#include "../common/sglist.h"
#include "../common/netbuf.h"
#include "../common/errptr.h"
#include <arch/sys_arch.h>
#include "netdev_driver.h"
 ...
 */
#define __U16_MAX   0xffff
#define NET_MAX_FRAGMENTS    ((__U16_MAX >> PAGE_SHIFT) + 2)
...
static void virtio_netdev_xmit_free(struct uk_netdev_tx_queue *txq)
{
	...
	debug("Free %"__PRIu16" descriptors\n", cnt);
}
				   ...
				   int notify)
{
	...
	while (filled < nb_desc) {
		...
		if (unlikely(cnt < req)) {
			debug("Incomplete fill-up of netbufs on receive virtqueue %p: Out of memory",
				    rxq);
			status |= UK_NETDEV_STATUS_UNDERRUN;
			goto out;
		}
...
out:
	debug("Programmed %"__PRIu16" receive netbufs to receive virtqueue %p (status %x)\n",
		    filled / 2, rxq, status);
			      ...
			      struct uk_netbuf *pkt)
{
	...
	if (unlikely(rc != 1)) {
		error("Failed to prepend virtio header\n");
		rc = -ENOSPC;
		goto err_exit;
	}
	vhdr = pkt->data;
	...
	if (unlikely(rc != 0)) {
		error("Failed to append to the sg list\n");
	...
	if (unlikely(total_len > VIRTIO_PKT_BUFFER_LEN)) {
		error("Packet size too big: %lu, max:%u\n",
	...
	if (likely(rc >= 0)) {
		...
		debug("No more descriptor available\n");
		/**
		 * Remove header before exiting because we could not send
		 */
		uk_netbuf_header(pkt, -header_sz);
	} else {
		error("Failed to enqueue descriptors into the ring: %d\n",
			  rc);
		goto err_remove_vhdr;
	}
	return status;
				     ...
				     struct uk_netbuf *netbuf)
{
	...
	if (unlikely(rc != 1)) {
		error("Failed to allocate space to prepend virtio header\n");
		return -EINVAL;
	}
	rxhdr = netbuf->data;
				     ...
				     struct uk_netbuf **netbuf)
{
	...
	if (ret < 0) {
		debug("No data available in the queue\n");
		*netbuf = NULL;
		return rxq->nb_desc;
	}
	if (unlikely((len < VIRTIO_HDR_LEN + UK_ETH_HDR_UNTAGGED_LEN)
		     || (len > VIRTIO_PKT_BUFFER_LEN))) {
		error("Received invalid packet size: %"__PRIu32"\n", len);
		return -EINVAL;
	}
			      ...
			      struct uk_netbuf **pkt)
{
	...
	ASSERT(!(queue->intr_enabled & VTNET_INTR_EN));

	rc = virtio_netdev_rxq_dequeue(queue, pkt);
	if (unlikely(rc < 0)) {
		error("Failed to dequeue the packet: %d\n", rc);
		goto err_exit;
	}
				...
				struct uk_netdev_rxqueue_conf *conf)
{
	...
	ASSERT(n);
	ASSERT(conf);
	ASSERT(conf->alloc_rxpkts);

	vndev = to_virtionetdev(n);
	if (queue_id >= vndev->max_vqueue_pairs) {
		error("Invalid virtqueue identifier: %"__PRIu16"\n",
			  queue_id);
		rc = -EINVAL;
		goto err_exit;
	}
	...
	rc = virtio_netdev_vqueue_setup(vndev, queue_id, nb_desc, VNET_RX,
					conf->a);
	if (rc < 0) {
		error("Failed to set up virtqueue %"__PRIu16": %d\n",
			  queue_id, rc);
		goto err_exit;
	}
	rxq  = &vndev->rxqs[rc];
				      ...
				      struct uk_netdev_queue_info *qinfo)
{
	...
	if (unlikely(queue_id >= vndev->max_vqueue_pairs)) {
		error("Invalid virtqueue id: %"__PRIu16"\n", queue_id);
		rc = -EINVAL;
		goto exit;
	}
				      ...
				      struct uk_netdev_queue_info *qinfo)
{
	...
	vndev = to_virtionetdev(dev);
	if (unlikely(queue_id >= vndev->max_vqueue_pairs)) {
		error("Invalid queue_id %"__PRIu16"\n", queue_id);
		rc = -EINVAL;
		goto exit;
	}
...
static int virtio_netdev_feature_negotiate(struct uk_netdev *n)
{
	...
	if (!VIRTIO_FEATURE_HAS(host_features, VIRTIO_NET_F_MAC)) {
		 ...
		 * MAC Address.
		 */
		error("%p: Host system does not offer MAC feature\n", n);
		rc = -EINVAL;
	...
	if (!VIRTIO_FEATURE_HAS(host_features, VIRTIO_NET_F_STATUS)) {
		error("%p: Host system does not offer MTU feature\n", n);
		rc = -EINVAL;
		goto err_negotiate_feature;
	}
	...
	if (!VIRTIO_FEATURE_HAS(host_features, VIRTIO_NET_F_CSUM)) {
		debug("%p: Host does not offer partial checksumming feature: Checksum offloading disabled.\n",
			    n);
	} else {
				    ...
				    const struct uk_netdev_conf *conf)
{
	...
	if (conf->nb_rx_queues != 1 || conf->nb_tx_queues != 1) {
		error("Queue combination not supported: %"__PRIu16"/%"__PRIu16" rx/tx\n",
	 ...
	 */
	vndev->rxqs = malloc(sizeof(*vndev->rxqs) * conf->nb_rx_queues);
	vndev->txqs = malloc(sizeof(*vndev->txqs) * conf->nb_tx_queues);
	if (unlikely(!vndev->rxqs || !vndev->txqs)) {
		error("Failed to allocate memory for queue management\n");
		rc = -ENOMEM;
		goto err_free_txrx;
	}
	...
	if (unlikely(vq_avail != total_vqs)) {
		error("Expected: %d queues, Found: %d queues\n",
			  total_vqs, vq_avail);
		rc = -ENOMEM;
		goto err_free_txrx;
	}
				   ...
				   const struct uk_netdev_conf *conf)
{
	...
	if (rc < 0) {
		error("%p: Failed to initialize rx and tx rings: %d\n",
			  n, rc);
	}
...
static int virtio_net_start(struct uk_netdev *n)
{
	...
	info(DRIVER_NAME": %"__PRIu16" started\n", d->uid);
...
/*
 * Before this function is invoked, vdev should have
 * the following fields setup:
 * - vdev->cops
 * - vdev->vdrv
 * - vdev->vqs
 */
static int virtio_net_add_dev(struct virtio_dev *vdev)
{
	...
	if (rc < 0) {
		error("Failed to register virtio-net device with libuknet\n");
	...
	debug("virtio-net device registered with libuknet\n");
...
void coffer_virtio_bus_driver_register()
{
	_virtio_bus_register_driver(&vnet_drv);
}